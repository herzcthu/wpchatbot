...
function saveUser(first_name, last_name, recipientId, locale, callback) {
  var name = first_name + ' ' + last_name;
  db.query('INSERT INTO users(name, recipientid, lang) values(?,?,?)', [name, recipientId, locale], function (err, rows) {
            if (err) return callback(err, null)
            return callback(null, rows)
          })
}
    ...
    message: {
      attachment: {
        ...
        payload: {
          ...
          elements: [{
            title: "Welcome to Myanmar WP",
            subtitle: "Build your site in Myanmar",
            item_url: "https://mya.wordpress.org",
            image_url: "https://mya.wordpress.org/files/2013/12/cropped-responsive2.png",
            buttons: [{
              type: "web_url",
              url: "https://mya.wordpress.org/txt-download/",
              title: "Download Myanmar Wordpress"
            }, {
              type: "postback",
              title: "Help",
              payload: "help",
...
function getUserProfile(userId) {
  request({
    ...
    if (!error && response.statusCode == 200) {
      var first_name = body.first_name, last_name = body.last_name, locale = body.locale;
      switch(locale) {
        case 'my_MM':
        case 'qz_MM':
        case 'en_US':
      ...
      saveUser(first_name, last_name, userId, lang, function(err, data){
        console.log(data);
      });
    } else {
      console.error("Failed calling Send API", response.statusCode, response.statusMessage, body.error);
